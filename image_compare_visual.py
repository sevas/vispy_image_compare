...
from vispy.visuals._scalable_textures import CPUScaledTexture2D, GPUScaledTexture2D
...
void main()
{
    ...
    if (split_method == 0) {
        ...
        if (texcoord.y < split_y) {
            c = texture2D(u_texture1, texcoord);
        }
        else {
            c = texture2D(u_texture2, texcoord);
        }    
    }else {
        // vertical split
        if (texcoord.x < split_x) {
            c = texture2D(u_texture1, texcoord);
        }
        else {  
            c = texture2D(u_texture2, texcoord);
    ...
    gl_FragColor = $color_transform(c);
}
"""  # noqa
...
class ImageCompareVisual(Visual):
    ...
    **kwargs : dict
    ...
    def __init__(self, data1=None, data2=None, method='auto', grid=(1, 1),
        ...
        self._texture1 = self._init_texture(data1, texture_format)
        self._texture2 = self._init_texture(data2, texture_format)
    ...
    def set_data(self, image1, image2, copy=False):
        """Set the image data.
        ...
        """
        data1 = np.array(image1, copy=copy)
        data2 = np.array(image2, copy=copy)
        ...
        self._texture1.check_data_format(data1)
        self._texture2.check_data_format(data2)

        if self._data1 is None or self._data1.shape[:2] != data2.shape[:2]:
            # Only rebuild if the size of the image changed
            self._need_vertex_update = True

        if self._data2 is None or self._data2.shape[:2] != data2.shape[:2]:
            # Only rebuild if the size of the image changed
            self._need_vertex_update = True
        self._data1 = data1
        self._data2 = data2
    ...
    def clim(self):
        ...
        return self._texture1.clim

    @clim.setter
    def clim(self, clim):
        if self._texture1.set_clim(clim):
            self._need_texture_upload = True
        if self._texture2.set_clim(clim):
            self._need_texture_upload = True
    ...
    def _update_colortransform_clim(self):
        if self._need_colortransform_update:
            ...
            return
        try:
            norm_clims = self._texture1.clim_normalized
        except RuntimeError:
            return
        else:
    ...
    def _build_interpolation(self):
        ...
        if self._texture1.interpolation != texture_interpolation:
            self._texture1.interpolation = texture_interpolation

        self._data_lookup_fn['texture1'] = self._texture1
        self._data_lookup_fn['texture2'] = self._texture2
    ...
    def _build_texture(self):
        try:
            pre_clims = self._texture1.clim_normalized
        except RuntimeError:
            pre_clims = "auto"
        pre_internalformat = self._texture1.internalformat
        # copy was already made on `set_data` if requested
        self._texture1.scale_and_set_data(self._data1, copy=False)
        self._texture2.scale_and_set_data(self._data2, copy=False)

        post_clims = self._texture1.clim_normalized
        post_internalformat = self._texture1.internalformat
        ...
        if new_if:
            ...
            # shortcut so we don't have to rebuild the whole color transform
            self.shared_program.frag['color_transform'][1]['clim'] = self._texture1.clim_normalized
    ...
    def _build_color_transform(self):
        if self._data1.ndim == 2 or self._data1.shape[2] == 1:
            # luminance data
        ...
        fclim['clim'] = self._texture1.clim_normalized
